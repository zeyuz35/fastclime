...
## Not run:
##D #generate data
##D L = fastclime.generator(n = 100, d = 20)
##D
##D #graph path estimation
##D out1 = fastclime(L$data,0.1)
##D out2 = fastclime.selector(out1$lambdamtx, out1$icovlist,0.2)
##D fastclime.plot(out2$adaj)
##D
##D #graph path estimation using the sample covariance matrix as the input.
##D out1 = fastclime(cor(L$data),0.1)
##D out2 = fastclime.selector(out1$lambdamtx, out1$icovlist,0.2)
##D fastclime.plot(out2$adaj)
## End(Not run)
