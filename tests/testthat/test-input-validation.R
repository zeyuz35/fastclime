...
test_that("paralp/fastlp validate NA/Inf inputs", {
  ...
  expect_error(paralp(c_na, A, b_lp, c_lp, b_lp), "obj must be a finite numeric vector")
  
  # Inf in mat
  A_inf <- A; A_inf[1,1] <- Inf
  expect_error(fastlp(c_lp, A_inf, b_lp), "mat must be a finite numeric matrix")
  expect_error(paralp(c_lp, A_inf, b_lp, c_lp, b_lp), "mat must be a finite numeric matrix")
  
  # Negative perturbation vector in paralp
  expect_error(paralp(c_lp, A, b_lp, c(-1, 1), b_lp), "The perturbation vector obj_bar and rhs_bar must be nonnegative")
  expect_error(paralp(c_lp, A, b_lp, c_lp, c(-1, 1, 1)), "The perturbation vector obj_bar and rhs_bar must be nonnegative")
})
