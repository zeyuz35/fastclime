...
void paralp(double *obj, double *mat, double *rhs, int *m0 , int *n0, double *opt, int *status, double *lambda, double *rhs_bar, double *obj_bar)
{
    int m = *m0;
    int n = *n0;
    int nz = 0;
    ...
    int *ia = (int*)FC_CALLOC((size_t)nz + m, sizeof(int));
    int *ka = (int*)FC_CALLOC((size_t)(size_t)n + m + 1, sizeof(int));
    double *a = (double*)FC_CALLOC((size_t)nz + m, sizeof(double));
    ...
    for (i=0; i<m; i++) {
        ...
        state->primal_B_bar[i] = rhs_bar[i];
    }
