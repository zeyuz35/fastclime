...
static void fc_lufac_init_wk(fc_lu_context_t *ctx, int m, int *kA, int *iA, double *A, int *basis, fc_lufac_work_t *wk) {
    ...
    if (ctx->kL == NULL)   { ctx->kL    = (int*)FC_CALLOC((size_t)m + 1,  sizeof(int)); } else { ctx->kL = (int*)FC_REALLOC(ctx->kL, (size_t)(size_t)m + 1, sizeof(int)); }
    ...
    if (ctx->kUt == NULL)  { ctx->kUt   = (int*)FC_CALLOC((size_t)m + 1,  sizeof(int)); } else { ctx->kUt = (int*)FC_REALLOC(ctx->kUt, (size_t)(size_t)m + 1, sizeof(int)); }
        ...
        for (k=ctx->kL[i]; k<ctx->kL[i+1]; k++) {
            ...
            for (kk=ctx->kUt[i]; kk<ctx->kUt[i+1]; kk++) {
                ...
                if ( wk->iwork[col2] == wk->tag ) {
                    ...
                    deg = wk->degBt[row2];
                    wk->Bt[row2] = (FC_VALIND*)FC_REALLOC( wk->Bt[row2], (size_t)(size_t)deg + 1, sizeof(FC_VALIND) );
        ...
        for (k=ctx->kUt[i]; k<ctx->kUt[i+1]; k++) {
            ...
            for (kk=ctx->kL[i]; kk<ctx->kL[i+1]; kk++) {
                ...
                if ( wk->iwork[row2] == wk->tag ) {
                    ...
                    deg = wk->degB[col2];
                    wk->B[col2] = (FC_VALIND*)FC_REALLOC( wk->B[col2], (size_t)(size_t)deg + 1, sizeof(FC_VALIND) );
...
static void fc_lufac_finalize_wk(fc_lu_context_t *ctx, int m, fc_lufac_work_t *wk) {
    ...
    if (ctx->kLt == NULL){ ctx->kLt= (int*)FC_CALLOC((size_t)m + 1, sizeof(int)); }           else { ctx->kLt= (int*)FC_REALLOC(ctx->kLt, (size_t)(size_t)m + 1, sizeof(int)); }
    ...
    if (ctx->kU == NULL) { ctx->kU = (int*)FC_CALLOC((size_t)m + 1, sizeof(int)); }           else { ctx->kU = (int*)FC_REALLOC(ctx->kU, (size_t)(size_t)m + 1, sizeof(int)); }
...
int fc_bsolve(fc_lu_context_t *ctx, int m, double *sy, int *iy, int *pny) {
    ...
    *pny = ny;
    ...
    if (ctx->enz + ny > FC_E_NZ) {
        ctx->E = (double*)FC_REALLOC(ctx->E, (size_t)(size_t)FC_E_NZ + ctx->enz + ny, sizeof(double));
        ctx->iE = (int*)FC_REALLOC(ctx->iE, (size_t)(size_t)FC_E_NZ + ctx->enz + ny, sizeof(int));
    }
