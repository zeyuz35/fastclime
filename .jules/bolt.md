...
## 2026-06-20 - Fast Matrix Symmetrization
**Learning:** In R, replacing `t(X) %*% X` with `crossprod(X)` is significantly faster and more memory-efficient than creating explicit transpose matrix and multiplying them across the whole matrix.
**Action:** When symmetrizing or combining matrices based on element-wise conditions, use `crossprod(X)` instead of `t(X) %*% X`.
