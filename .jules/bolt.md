...
**Learning:** Explicitly computing `t(X) %*% X` creates an unnecessary intermediate transposed matrix in memory, which is computationally expensive and memory-intensive for large datasets.
**Action:** Always prefer base R's built-in `crossprod(X)` (and `tcrossprod()`) for symmetric matrix operations as it avoids the transposition overhead.
