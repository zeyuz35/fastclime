...
## 2024-04-25 - Optimize eigenvalue extraction for symmetric matrices
**Learning:** Extracting eigenvalues from large symmetric matrices without specifying `symmetric = TRUE` defaults to generic routines that are an order of magnitude slower.
**Action:** Always use `eigen(x, symmetric = TRUE, only.values = TRUE)` when only eigenvalues are needed for symmetric matrices to bypass O(n^3) computations.
