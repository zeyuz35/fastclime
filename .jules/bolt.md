...
## 2024-05-06 - Eigenvalue computation optimization
**Learning:** When calculating eigenvalues of a symmetric matrix, using `eigen(x, symmetric = TRUE, only.values = TRUE)` skips the O(n^3) computation of eigenvectors.
**Action:** When only eigenvalues are needed for symmetric matrices, strictly use `symmetric = TRUE, only.values = TRUE`.
