...
## 2024-06-12 - Optimize Matrix Cross-Product
**Learning:** In R, evaluating `t(X) %*% X` explicitly transposes the matrix and then performs matrix multiplication, which is memory and compute inefficient.
**Action:** Always use `crossprod(X)` instead of `t(X) %*% X` to optimize calculation of covariance or Gram matrices.
