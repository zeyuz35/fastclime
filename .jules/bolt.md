...
## 2024-05-18 - Avoid redundant solve() by scaling precision matrix directly
**Learning:** In R mathematical computations, avoid redundant `solve()` (matrix inversion) calls, as they are expensive O(N^3) operations. If the covariance matrix is scaled by its diagonal D, its inverse (the precision matrix) can be derived by directly scaling the original precision matrix \Omega using D^{1/2} \Omega D^{1/2} (e.g., via fast vector-matrix multiplication `t(t(omega * D_sqrt) * D_sqrt)`).
**Action:** Always check if a mathematical relationship can avoid a full matrix inversion in favor of simple matrix scaling or fast vector-matrix operations.
