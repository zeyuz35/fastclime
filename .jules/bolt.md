...
## 2026-06-09 - Avoid explicit transpose with crossprod
**Learning:** In R, the `t(X) %*% X` operation is inefficient due to the explicit allocation of the transposed matrix.
**Action:** Always replace `t(X) %*% X` with `crossprod(X)` (and `X %*% t(Y)` with `tcrossprod(X, Y)`) for improved performance and reduced memory overhead.
