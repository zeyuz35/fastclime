...
## 2024-05-24 - Faster Eigenvalues for Symmetric Matrices
**Learning:** When calculating eigenvalues for covariance or precision matrices, which are symmetric by definition, the `eigen()` function can be significantly sped up by explicitly setting `symmetric = TRUE`.
**Action:** Always set `symmetric = TRUE` in `eigen()` when working with known symmetric matrices to bypass general matrix eigenvalue computation.
