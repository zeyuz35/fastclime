...
## 2026-06-14 - Replace t(X) %*% X with crossprod(X)
**Learning:** Base R operations like `t(X) %*% X` allocate an explicit matrix copy in memory during the transpose, causing significant memory and CPU overhead.
**Action:** Always replace `t(X) %*% X` with base R's optimized `crossprod(X)` to bypass allocating the transpose.
