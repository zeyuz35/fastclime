...
## 2026-04-10 - O(n^3) Eigenvector Computation in Generator
**Learning:** Using `eigen()$values` on a large symmetric matrix computes eigenvectors by default, which is an unnecessary O(n^3) operation when only checking matrix positivity via eigenvalues.
**Action:** Always use `eigen(x, symmetric = TRUE, only.values = TRUE)$values` when only eigenvalues of a symmetric matrix are needed.
