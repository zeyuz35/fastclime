...
## 2024-05-20 - Faster covariance matrix calculation
**Learning:** `t(X) %*% X` explicitly calculates and allocates the transpose of `X`, which is memory-intensive and computationally expensive. `crossprod(X)` computes the exact same result directly, without forming the transpose.
**Action:** When calculating sample covariance matrices or $X^TX$ operations, use `crossprod(X)` instead of `t(X) %*% X`.
