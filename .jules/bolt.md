...
## 2026-06-18 - Fast matrix cross-products
**Learning:** In R, `crossprod(X)` is computationally faster and requires less memory allocation than manually transposing and multiplying `t(X) %*% X`.
**Action:** When calculating sample covariance or Gram matrices, prefer `crossprod()` over manual matrix multiplication for improved performance.
