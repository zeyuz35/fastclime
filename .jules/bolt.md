...
## 2026-06-17 - Fast Matrix Cross-Products
**Learning:** In R, computing the cross-product of a matrix with itself using `crossprod(X)` is faster and more memory-efficient than the naive matrix multiplication `t(X) %*% X`, because it avoids explicitly creating the transposed matrix in memory and uses optimized BLAS routines.
**Action:** Always prefer `crossprod(X)` or `crossprod(X, Y)` over `t(X) %*% X` or `t(X) %*% Y` for performance-critical matrix operations.
